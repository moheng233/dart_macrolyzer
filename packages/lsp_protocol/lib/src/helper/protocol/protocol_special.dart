...
import '../../protocol/protocol_custom_generated.dart';
import '../../protocol/protocol_generated.dart';
import '../lsp/json_parsing.dart';
...
Null _alwaysNull(_, [_]) => null;

bool _alwaysTrue(_, [_]) => true;
...
class Either3<T1, T2, T3> implements ToJsonable {
  const Either3.t1(this._t1) : _t2 = null, _t3 = null, _which = 1;

  const Either3.t2(this._t2) : _t1 = null, _t3 = null, _which = 2;

  const Either3.t3(this._t3) : _t1 = null, _t2 = null, _which = 3;
...
class Either4<T1, T2, T3, T4> implements ToJsonable {
  const Either4.t1(this._t1) : _t2 = null, _t3 = null, _t4 = null, _which = 1;

  const Either4.t2(this._t2) : _t1 = null, _t3 = null, _t4 = null, _which = 2;

  const Either4.t3(this._t3) : _t1 = null, _t2 = null, _t4 = null, _which = 3;

  const Either4.t4(this._t4) : _t1 = null, _t2 = null, _t3 = null, _which = 4;
...
class ErrorOr<T> extends Either2<ResponseError, T> {
  const ErrorOr.error(super.error) : super.t1();

  const ErrorOr.success(super.result) : super.t2();

  /// Returns the error or throws if object is not an error. Check [isError]
  /// before accessing [error].
  ResponseError get error {
    return _which == 1 ? _t1! : (throw StateError('Value is not an error'));
  }

  /// Returns true if this object is an error, false if it is a result. Prefer
  /// [mapResult] instead of checking this flag if [error] will simply be
  /// propagated as-is.
  ...
  T get result {
    return _which == 2 ? _t2 as T : (throw StateError('Value is not a result'));
  }
...
// ignore: one_member_abstracts must
