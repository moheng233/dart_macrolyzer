...
// ignore_for_file: avoid_print, prefer_asserts_with_message util
...
Future<void> main(List<String> arguments) async {
  ...
  final packageFolder = File(script).parent.parent.path;
  final outFolder = path.join(packageFolder, 'lib', 'src', 'protocol');
...
List<LspEntity> getCustomClasses() {
  ...
  final customTypes = <LspEntity>[
    TypeAlias(name: 'LSPAny', baseType: TypeReference.LspAny, isRename: false),
    TypeAlias(
      name: 'LSPObject',
    ...
    TypeAlias(name: 'URI', baseType: TypeReference('LSPUri'), isRename: true),
    TypeAlias(name: 'LSPUri', baseType: TypeReference('Uri'), isRename: false),
...
Future<List<LspEntity>> getSpecClasses(ArgResults args) async {
  final download = args[argDownload] as bool;
  if (download) {
    await downloadSpec();
  }
...
class AbstractGetter extends Member {
  AbstractGetter({required super.name, required this.type, super.comment});
