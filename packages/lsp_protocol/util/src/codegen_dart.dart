...
// ignore_for_file: avoid_print util
...
String generateDartForTypes(List<LspEntity> types) {
  ...
  for (final type in fileSortedTypes) {
    _writeType(buffer, type);
  }
  for (final function in _canParseFunctions.values) {
    buffer.writeln(function);
  }
  for (final function in _unionFunctions.values) {
    buffer.writeln(function);
  }
...
void recordTypes(List<LspEntity> types) {
  ...
  types.whereType<Interface>().forEach((interface) {
    ...
    for (final base in interface.baseTypes) {
      _subtypes[base.dartType] ??= <String>[interface.name];
    }
  });
...
TypeBase resolveTypeAlias(
  ...
  if (type is TypeReference) {
    ...
    if (alias != null && (!onlyRenames || alias.isRename)) {
      ...
      for (var i = 0; i < 10; i++) {
        final newResolved = resolveTypeAlias(
          resolved,
      ...
      throw StateError('Failed to resolve type after 10 iterations: ${alias.name}');
    }
  }
  return type;
}
...
String _determineVariableName(
  ...
  final fieldNames = _getAllFields(interface).map((f) => f.name).toList();
  final suggestion = suggestions.firstWhereOrNull(
    (s) => !fieldNames.contains(s),
  );
  if (suggestion != null) {
    return suggestion;
  }
  final first = suggestions.firstOrNull ?? 'var';
  for (var i = 1; true; i++) {
    final suggestion = '$first$i';
...
String _getTypeCheckFailureMessage(TypeBase type) {
  final ntype = resolveTypeAlias(type);

  if (ntype is LiteralType) {
    return r"must be the literal '$literal'";
  } else if (ntype is LiteralUnionType) {
    return "must be one of the \${literals.map((e) => \"'\$e'\").join(', ')}";
  } else {
    return 'must be of type ${ntype.dartTypeWithTypeArgs}';
  }
}

bool _isOverride(Interface interface, Field field) {
  for (final parentType in interface.baseTypes) {
    final parent = _interfaces[parentType.name];
    if (parent != null) {
...
bool _isSpecType(TypeBase type) {
  final resolvedType = resolveTypeAlias(type);
  return resolvedType is TypeReference &&
      resolvedType != TypeReference.LspObject &&
      resolvedType != TypeReference.LspAny &&
      (_interfaces.containsKey(resolvedType.name) ||
          (_namespaces.containsKey(resolvedType.name)));
}

bool _isUriType(TypeBase type) {
  final resolvedType = resolveTypeAlias(type);
  return resolvedType is TypeReference && resolvedType.dartType == 'Uri';
}
...
String _memberNameForType(TypeBase type) {
  ...
  final resolvedType = type is TypeReference ? resolveTypeAlias(type) : type;

  final dartType = resolvedType is NullableType
      ? '${resolvedType.dartType}?'
      : resolvedType is UnionType
      ? resolvedType.types.map(_memberNameForType).join()
      : resolvedType is ArrayType
      ? 'List${_memberNameForType(resolvedType.elementType)}'
      : resolvedType is MapType
      ? 'Map${_memberNameForType(resolvedType.indexType)}${_memberNameForType(resolvedType.valueType)}'
      : resolvedType.dartType;
...
Iterable<String> _wrapLines(List<String> lines, int maxLength) sync* {
  ...
  for (var line in trimmedLines) {
    while (true) {
      if (line.length <= maxLength) {
        yield line;
        break;
      } else {
        final lastSpace = line.lastIndexOf(' ', maxLength);
...
void _writeCanParseMethod(IndentableStringBuffer buffer, Interface interface) {
  ...
  for (var i = 0; i < fields.length; i++) {
    ...
    final functionName = '_canParse${_memberNameForType(type)}';
    final invocation =
    ...
    if (!_canParseFunctions.containsKey(functionName)) {
      final temp = IndentableStringBuffer();
...
void _writeCanParseType(
  ...
  final failureMessage = _getTypeCheckFailureMessage(type);
  final quote = failureMessage.contains("'") ? '"' : "'";
...
void _writeConstructor(IndentableStringBuffer buffer, Interface interface) {
  ...
  if (fieldsWithValidation.isNotEmpty) {
    buffer
      ..writeIndentedln(' {')
      ..indent();
    for (final field in fieldsWithValidation) {
      final type = field.type;
      if (type is LiteralType) {
        buffer
          ...
          ..indent()
          ..writeIndentedln(
            "throw '${field.name} may only be the literal ${type.valueAsLiteral.replaceAll("'", r"\'")}';",
          )
          ..outdent()
          ..writeIndentedln('}');
      }
...
void _writeDocCommentsAndAnnotations(
  ...
  if (comment != null && comment.isNotEmpty) {
    ...
    final originalLines = comment.split('\n');
    // Wrap at 80 - 4 ('/// ') - indent characters.
    final wrappedLines = _wrapLines(
      originalLines,
      (80 - 4 - buffer.totalIndent).clamp(0, 80),
    );
    for (final l in wrappedLines) {
...
void _writeEquals(IndentableStringBuffer buffer, Interface interface) {
  buffer
    ...
    ..indent()
    ..writeIndented('');
  for (final field in _getAllFields(interface)) {
    buffer.write(' && ');
...
void _writeInterface(IndentableStringBuffer buffer, Interface interface) {
  ...
  final allBaseTypes = interface.baseTypes
...
void _writeToJsonFieldsForResponseMessage(
  ...
  final allFields = _getAllFields(interface);
  final standardFields = allFields.where(
...
void _writeTypeCheckCondition(
  ...
  if (isNullableAnyType(type)) {
    ...
    throw ArgumentError('Unable to type check $valueCode against $fullDartType');
  }
}
